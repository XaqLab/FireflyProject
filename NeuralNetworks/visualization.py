...
def plot_trajectories(game, trajectories, fireflies):
    """ Plot a trajectory. """
    n = len(trajectories)
    if n == 1:
...
def plot_objective_function(game):
    ...
    plt.show(block=False)
    # reset weights to w0
    game.tf_session.run(set_weights, feed_dict={new_weights:w0})
...
def plot_network_response(game):
    ...
    for i in range(len(directions)):
        feed = game.feed_dict(np.array([[directions[i], 0.0]]))
        rotation[i] = game.tf_session.run(game.rotation, feed_dict=feed)
    for i in range(len(distances)):
        feed = game.feed_dict(np.array([[0.0, distances[i]]]))
        step_size[i] = game.tf_session.run(game.step_size, feed_dict=feed)
    fig1 = plt.figure()
